...
:::note
This is different from the top-level `serviceName` (headless service for StatefulSets) and `service.name` (name of the Service resource). This `ingress.serviceName` only controls which service the ingress routes traffic to.
:::
...
## `ingressClassName`
The `ingressClassName` option expects a string with the Kubernetes ingress class name.
...
## `defaultBackend`
The `defaultBackend` option is optional and expects a Kubernetes Ingress default backend object.
...
## `certmanager`
The `certmanager` option controls whether cert-manager annotations are added automatically when TLS is enabled.

#### Default Value For `certmanager`
```yaml
certmanager: true
```
