...
1. The first container would be create from the image `dscr.io/username/my-backend-image:1.15`
2. The second container would be created from the `busybox` image on [Docker Hub](https://hub.docker.com) which is tagged as version `latest` (default value)
...
## `imagePullPolicy`
The `imagePullPolicy` option is optional and expects a Kubernetes container image pull policy.
...
## `workingDir`
The `workingDir` option is optional and expects a string with the container working directory.
...
## `stdinOnce`
The `stdinOnce` option is optional and expects a boolean which defines if stdin should be closed after one attached client disconnects.

## `tty`
The `tty` option is optional and expects a boolean which defines if tty should be enabled for this container.

## `terminationMessagePath`
The `terminationMessagePath` option is optional and expects a string with the path where the container termination message is written.

## `terminationMessagePolicy`
The `terminationMessagePolicy` option is optional and expects a Kubernetes termination message policy.
...
## `envFrom`
The `envFrom` option is optional and expects an array of Kubernetes `envFrom` sources.
...
## `ports`
The `ports` option is optional and expects an array of Kubernetes container ports. If a port name is not set, the chart uses `port-[index]`.
...
## `securityContext`
The `securityContext` option is optional and expects a Kubernetes container security context.
...
## `lifecycle`
The `lifecycle` option is optional and expects a Kubernetes container lifecycle configuration.
...
## `volumeMounts`
The `volumeMounts` option expects an array of volumes which should be mounted into this container.
...
## `volumeDevices`
The `volumeDevices` option is optional and expects an array of Kubernetes volume devices.
...
- Ephemeral (non-persistent container) storage in Gi (Gigabyte), Mi (Megabyte) or Ki (Kilobyte)
- NVIDIA GPUs via `gpu`, rendered as `nvidia.com/gpu`
...
- [Ingress](./ingress.mdx) : Configure ingress for this release.
- [ConfigMap](./configmap.mdx) : Create a Kubernetes `ConfigMap` for this release.
- [Secrets](./secrets.mdx) : Create a Kubernetes `Secret` for this release.
- [Volumes](./volumes.mdx) : Define persistent volumes for this release.
- [Service Account](./serviceaccount.mdx) : Create a Kubernetes `ServiceAccount` for this release.
- [Extra Objects](./extra-objects.mdx) : Render additional Kubernetes objects **verbatim**.
