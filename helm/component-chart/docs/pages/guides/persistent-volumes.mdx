...
## Define Volumes
You can define the following types of Kubernetes volumes:
...
### `emptyDir` Volumes
You can define temporary volumes to share files between containers of a Pod according to [Kubernetes EmptyDirVolumeSource specification](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.22/#emptydirvolumesource-v1-core)
```yaml
volumes:
- name: temp-storage
  emptyDir: {}
...
### ConfigMap Volumes
You can define Kubernetes ConfigMaps as volumes according to the [Kubernetes ConfigMapVolumeSource specification](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.22/#configmapvolumesource-v1-core):
```yaml
volumes:
- name: nginx-config
  configMap:
...
### Secret Volumes
You can define Kubernetes Secrets as volumes according to the [Kubernetes SecretVolumeSource specification](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.22/#secretvolumesource-v1-core):
```yaml
volumes:
- name: secret-token
  secret:
...
### HostPath Volumes
You can define your Node's filesystem as volumes according to the [Kubernetes HostPath specification](https://kubernetes.io/docs/concepts/storage/volumes/#hostpath):
```yaml
volumes:
- name: docker-sock
  hostPath:
...
The above example will mount your local Docker socket to a socket inside the container, which allows for Docker In Docker (DIND) 


<details>
<summary>Show specification for HostPath volumes</summary>

```yaml
volumes:
- name: [a-z0-9-]{1,253}            # Name of the volume (used to mount the volume)
  hostPath:                         # Kubernetes HostPathVolumeSource v1
    path: [a-z0-9-]{1,253}          # File or Directory on the node's filesystem
    ...
```
